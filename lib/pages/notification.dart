...
class _NotificationsPageState extends State<NotificationsPage> {
  ...
  //   try {
  //     int patientId =
  //         await apiService.fetchPatientIdd(patientCode, widget.username);
  //     final patientDetail =
  //         await ApiService().getPatientById(patientId.toString());
  ...
  void handleNotificationTap(NotificationDetail notification) async {
    ...
    try {
        // This method now returns void and throws errors if something goes wrong
        await notificationService.changeNotificationFlagRead(widget.username, notification.id);
        // Update the UI state to reflect the change if no error is thrown
        setState(() {
            notification.flagRead = true;
        });

        // Proceed with further actions only if the flag update was successful
        String patientCode = notification.parsePatientCode();
        ...
        // If there's an error updating the flag or fetching patient data, handle it here
        print("Error in handling notification tap: $e");
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("Failed to handle notification tap: $e"))
        );
    }
}
                          ...
                          children: [
                            ListTile(
                                    ...
                                    children: [
                                      ...
                                      Positioned(
                                        ...
                                        child: CircleAvatar(
                                          ...
                                          backgroundImage: AssetImage(
                                              'assets/ic_edit.png'), // Small image
                                        ),
                                      ),
                                    ],
                                  ),
                              ...
                              title: Text(notification.title,
                                  style: TextStyle(
                                    fontWeight: notification.flagRead
                                        ? FontWeight.normal
